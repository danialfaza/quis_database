...
class MainActivity : AppCompatActivity() {
    ...
    fun deleteAllRecord(view: View) {
        //Buat dialog untuk validasi apakah akan menghapus semua
        val dialogBuilder = AlertDialog.Builder(this)
        dialogBuilder.setTitle("Hapus Semua Data")
        dialogBuilder.setMessage("Anda yakin ingin menghapus semua?")
        dialogBuilder.setPositiveButton("Hapus", DialogInterface.OnClickListener { _, _ ->

            val databaseHandler: DatabaseHandler= DatabaseHandler(this)
            val status = databaseHandler.deleteAllEmployee()
                if(status > -1){
                    Toast.makeText(applicationContext,"Semua data telah hilang",Toast.LENGTH_LONG).show()
                    viewRecord()
                }
        })
        dialogBuilder.setNegativeButton("Cancel", DialogInterface.OnClickListener { _, _ ->
